...
The latest real compiled compatibility probe is temporary PR #226 on post-M4 audit main with CodeQL action `4.38.1`. Run `35616437045` / job `106388233409` resolved stable CodeQL CLI `2.27.0`; initialization succeeded, but the real `:app:assembleDebug` trace failed at `:core:model:compileKotlin` with `KotlinVersionTooRecentError` because Kotlin `2.4.20` is still too recent for that extractor. PR #226 was closed unmerged after recording the evidence, so the automatic compatibility gate remains in place until a later real probe succeeds.
