...
Open-Meteo requests also explicitly pin `cell_selection=land`. This matches the selection behavior used by the measured M0 baseline and the provider's current Forecast API default, preserving land/DEM-aware cell selection while preventing a future provider-default change from silently changing MeteoOne request semantics. M1 does not switch the baseline to raw nearest-grid selection or disable Open-Meteo elevation downscaling without new measured evidence.
...
The consumer-facing `:forecast:data` surface is the `M1ForecastEngine` façade, `M1ForecastEngineResult`, and `M1ForecastSourceIdentity`. Available results expose the canonical `FusedForecast` plus successful/failed source identities using only `:core:model` and `:forecast:data` types. Provider request planners and DTOs, HTTP adaptation, Open-Meteo mapping, direct-source parsing, GRIB decode values and semantics, native/ecCodes integration, decompression, provider-grid selection, and domain orchestration types are module implementation details and are not supported consumer APIs.

The Android `:app` module declares its production forecast dependency on `:forecast:data`, not directly on `:forecast:domain`. `:forecast:data` depends on `:forecast:domain` with Gradle `implementation`, so domain orchestration is no longer transitively exported to application consumers. Repository policy tests enforce both sides of that boundary.
